...
The game is loosely based on the analog dice-game ["Zombie Dice"](http://www.sjgames.com/dice/zombiedice/) and turned into this single player "push-your-luck" terminal game with a new theme and a few different rules.
...
This i what is first shown upon running the game. A simple explanation of the rules with a little bit of thematic flavour.
...
Every time the user rolls, the dice that are left and what colour they are is printed so that the user can make a decision if the risk is worth it to continue. Remember, the different coloured dice have different risks and rewards!
...
If the user does not push their luck beyond the limit and wants to end with the treasure collected, they will get a message with the final score. But could they have gotten more? 

![End game](https://i.imgur.com/q0PwWwW.png)

### No dice left

If the player survives through all six rooms, the game gives this end message that tells the player that they have completed the tomb and gives the final score. 
...
There are 12 dice, each with six sides, in the game. Each die has a colour that represents the number of traps, treasures, and empty chests that the user can get by rolling them.

- Green Dice: This is the most favourable die, with 3 treasures, 2 empty, and only 1 trap.

- Yellow Dice: This is the middle-of-the-line die, with 2 treasures, 2 empty, and 2 traps.

- Red Dice: The most dangerous die, with 3 traps, 2 empty, and only 1 treasure.
...
At every step, there has been rigorous manual testing to make sure the functions work as intended.
...
This error came up sometimes when rolling the dice because of the way the code was written, the random function could choose one number outside of the intended list. This was fixed by adding " - 1" to the affected lines.

![of range](https://i.imgur.com/VI8rxZk.png)

### Total traps are not updating

After testing and adding some code, the total traps did not add up when traps were triggered.
...
9. Repeat step 8 to add node.js. o Note: The Buildpacks must be in the correct order. If not, click and drag them to move them into the correct order
...
Family and friends that have supported my studies, and my mentor Anto Rodriguez, who is a reliable pillar of support through the projects.